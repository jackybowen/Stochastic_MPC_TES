...
function run(GUI_input::Dict)
    ...
    for z = 1:Int(numzones)
        ...
        insertcols!(result6,z,Symbol("Tupp_$z")=>vcat(Tzmax[z,1],sol_Tz_upp[z,:]))
        insertcols!(result7,z,Symbol("Tlow_$z")=>vcat(Tzmin[z,1],sol_Tz_low[z,:]))
    end
    result0 = DataFrame(price = vcat(price[min_idx[t0]], vec(sol_pr)), OutdoorTemperature = vcat(T_oa,vec(T_oa_p)))
    result = hcat(result0,result1,result2,result3,result4,result6,result7)
	# println(T_noise)
    return result;
