...
outdoortemp = ['Environment:Site Outdoor Air Drybulb Temperature [C](TimeStep)']
Toa = raw_data[['Date/Time']+outdoortemp]
Toa.to_csv('baseline_oat.csv',index=False)
...
Tsetpoint = raw_data[['Date/Time']+tempsetpoints]
...
Tdischarge = raw_data[tdiss]
Minmax = Tdischarge.agg([min, max])
Minmax.to_csv('baseline_tempdisbounds.csv')
Tdischarge.to_csv('baseline_tempdischarge.csv',index=False)
